...
# Copyright (C) 2017 MarcoMarinho for Euclidean OS
#
...
release() {
  ...
  export EUCOS_BUILD_TYPE=NIGHTLY
  cd ${SOURCE}
  ...
  do
    brunch ${DEVICE}
    mv ${SOURCE}/out/target/product/${DEVICE}/eucos-*-${DEVICE}-*.zip ${OUTDIR}
    mka clean
  done
}
...
echo ""
echo "${red}=========================================================${reset}"
echo "${red}==${reset}${green}            Euclidean OS Release Script              ${reset}${red}==${reset}"
...
echo "${red}==${reset}${yellow}   3 - Build Google Nexus 5 Test Builds              ${reset}${red}==${reset}"
