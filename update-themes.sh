...
function copy_all () {
  ...
  do
    cd $WORKING_DIR/packages/apps
    cp -r ${array[i]} $WORKING_DIR/resources/themes-resources/packages/apps/
  done
}

declare -a root=('resources' 'abi' 'bionic' 'art' 'bootable' 'build' 'dalvik' 'development' 'device' 'external' 'frameworks' 'hardware' 'kernel' 'libcore' 'libnativehelper' 'manifest' 'ndk' 'out' 'packages' 'pdk' 'prebuilts'
...
declare -a values=('all_search_engines.xml' 'appmsg_colors.xml' 'arrays.xml' 'attrs_manifest.xml' 'bookmarks_icons.xml' 'bools.xml' 'config.xml' 'defaults.xml' 'donottranslate.xml' 'donottranslate_config.xml'
                   ...
                   'donottranslate-search_engines.xml' 'du_arrays.xml' 'du_strings.xml' 'purenexus_symbols.xml' 'fractions.xml' 'ids.xml' 'integers.xml' 'internal.xml' 'keys.xml' 'lland_config.xml' 'lland_strings.xml' 'plurals.xml'
                   'public.xml' 'strings.xml' 'symbols.xml' 'vpi_attrs.xml' 'vpi_defaults.xml' 'purenexus_arrays.xml' 'custom_strings.xml' 'custom_arrays.xml' 'vpi__defaults.xml' 'vpi__attrs.xml'
                   'nontranslatables.xml' 'accountprovider.xml' 'constants.xml' 'purenexus_strings' 'filtershow_ids.xml' 'filtershow_strings.xml' 'filtershow_values.xml' 'filtershow_values_attrs.xml')

declare -a core=('java' 'jni' 'tests' 'src')

declare -a theme_packages=('ContactsCommon' 'PureNexusSettings' 'OpenDelta' 'PhoneCommon' 'Settings' 'Stk')
...
git clone https://github.com/EuclideanOS/resources.git
...
cd values
delete_useless ${values[@]}
rm -rf  $WORKING_DIR/resources/themes-resources/frameworks
cd $WORKING_DIR
process_all ${theme_packages[@]}
echo "Cleaning target folders"
rm -rf resources/themes-resources/packages
mkdir resources/themes-resources/packages
mkdir resources/themes-resources/packages/apps
mkdir resources/themes-resources/frameworks
echo "Copying all files to $WORKING_DIR/resources/themes-resources"
copy_all ${theme_packages[@]}
cp -r $WORKING_DIR/frameworks/base/core $WORKING_DIR/frameworks/base/packages $WORKING_DIR/resources/themes-resources/frameworks
cd $WORKING_DIR/resources/themes-resources
delete_useless ${res[@]}
cd $WORKING_DIR/resources/themes-resources
